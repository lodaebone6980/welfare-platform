...
const KEYWORDS: Array<{ key: string; label: string; emoji: string; q: string }> = [
...
async function getAllFeeds(): Promise<Feed[]> {
  return Promise.all(
    KEYWORDS.map(async (k) => {
      try {
        const items = await fetchNewsRSS(k.q)
        return {
          key: k.key,
          label: k.label,
          emoji: k.emoji,
          description: `Google News RSS · "${k.q}"`,
          items,
        }
      } catch (e: any) {
        return {
...
export default async function SearchTrendingPage() {
  const feeds = await getAllFeeds()
  const totalItems = feeds.reduce((s, f) => s + f.items.length, 0)
    ...
    <div className="p-4 sm:p-6 lg:p-8 xl:p-10 max-w-[1600px] mx-auto w-full">
      <div className="mb-6">
        ...
        <p className="text-xs text-gray-500 mt-1">
          Google News RSS를 수집해 복지·지원금·정책 관련 최신 트렌드를 보여드립니다. · 10분 캐시 · 총 {totalItems}건 {failed > 0 && <span className="text-red-500">· 실패 {failed}</span>}
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {feeds.map((f) => (
          <div key={f.key} className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-sm transition">
            ...
            {f.error ? (
              <p className="text-xs text-red-500 py-2 bg-red-50 rounded px-2">수집 실패: {f.error}</p>
            ) : f.items.length === 0 ? (
              <p className="text-xs text-gray-400 py-2">데이터 없음</p>
            ) : (
              <ul className="space-y-2.5">
                {f.items.slice(0, 6).map((it, i) => (
