...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // canonical: canonicalId 있으면 대표원본 URL, 없으면 자기 URL
      ...
      <div className="max-w-3xl mx-auto px-4 pb-24">
        ...
        {policy.applyUrl && (
          <div className="mb-8 text-center">
            <TrackedExternalLink href={policy.applyUrl} eventName="apply_click" metadata={{ policy_id: policy.id, policy_slug: policy.slug }} rel="nofollow" className="inline-flex items-center gap-2 bg-blue-600 text-white px-16 py-3.5 rounded-xl font-semibold text-base hover:bg-blue-700 transition-colors shadow-lg min-w-[260px] justify-center">
              신청하기
        ...
        <section id="eligibility" className="mb-8">
          ...
          <div className="bg-green-50 rounded-xl border border-green-100 p-5">
            {policy.eligibility ? (
              <div className="prose prose-sm max-w-none text-gray-700 prose-li:my-0.5 prose-ul:my-2 prose-ol:my-2 prose-p:my-2" dangerouslySetInnerHTML={{ __html: normalizePolicyHtml(policy.eligibility) }} />
            ) : (
        ...
        <section id="howto" className="mb-8">
          ...
          <div className="bg-white rounded-xl border p-5">
            {policy.applicationMethod ? (
              <div className="prose prose-sm max-w-none text-gray-700 prose-li:my-0.5 prose-ul:my-2 prose-ol:my-2 prose-p:my-2" dangerouslySetInnerHTML={{ __html: normalizePolicyHtml(policy.applicationMethod) }} />
            ) : (
        ...
        {policy.requiredDocuments && (
          <section className="mb-8">
            ...
            <div className="bg-purple-50 rounded-xl border border-purple-100 p-5">
              <div className="prose prose-sm max-w-none text-gray-700 prose-li:my-0.5 prose-ul:my-2 prose-ol:my-2 prose-p:my-2" dangerouslySetInnerHTML={{ __html: normalizePolicyHtml(policy.requiredDocuments) }} />
            </div>
          </section>
        )}
        ...
        <div className="fixed bottom-16 left-0 right-0 bg-white border-t shadow-lg px-4 py-3 z-20">
          <div className="max-w-3xl mx-auto flex gap-3">
            {policy.applyUrl ? (
              <TrackedExternalLink href={policy.applyUrl} eventName="apply_click" metadata={{ policy_id: policy.id, policy_slug: policy.slug }} rel="nofollow" className="flex-1 bg-blue-600 text-white text-center py-3 rounded-xl font-medium hover:bg-blue-700 transition-colors">
                신청하기
              </TrackedExternalLink>
            ) : (
