...
[applicationMethod 필드 — H2 없이, 콜론 라벨 없이, 단계 마커 없이]
<ol class=\"steps\"><li>서울청년포털 youth.seoul.go.kr에 접속해 회원가입 후 간편인증으로 로그인합니다.</li><li>메인 화면에서 \"청년수당\" 모집 공고를 확인하고 신청기간·자격·서류를 점검합니다.</li>...</ol>
중요 금지사항:
- <li>1단계. ...</li> 처럼 \"1단계.\"·\"2단계.\"·\"①\"·\"1.\" 같은 단계 마커 텍스트 사용 금지(ol이 자동으로 1./2./3. 번호를 매김 → 텍스트 마커는 중복).
- <strong>...</strong> 단계 강조 금지. 그냥 li 안에 자연 문장.

[requiredDocuments 필드 — H2 없이, 분류별 단락 3개로]
<p>기본 서류로 신분증 사본, 주민등록등본 최근 3개월 이내, 건강보험 자격득실확인서, 가족관계증명서(상세, 본인 기준), 본인 명의 통장 사본이 필요합니다.</p>
<p>본인 상황에 따라 근로·사업·기타소득 증빙(근로계약서·원천징수영수증), 주 30시간 이하 근로계약서, 군 복무 사실 증명서, 한부모·차상위 등 특수계층 증빙을 추가로 제출합니다.</p>
<p>모든 서류는 정부24·국민건강보험공단·홈택스·병무청에서 온라인 무료 발급이 가능합니다.</p>
중요 금지사항:
- <strong>필수:</strong>·<strong>해당 시:</strong> 콜론 라벨 사용 금지
- ul 사용 금지
- \"필요 서류는 다음과 같습니다\", \"제출 서류는 ~입니다\" 같은 redundant 도입부 문장 금지
- 첫 단락은 기본 서류, 두 번째는 추가 서류, 세 번째는 발급처 안내 — 분류별로 <p> 3개.
...
export function postProcessHtml(html: string): string {
  ...
  // 6. ol class=\"steps\"의 li 시작 \"1단계.\" / \"2단계 .\" / \"①\" / \"1.\" 단계 마커 제거 (ol 자동 번호와 중복)
  s = s.replace(
    /<li>\s*(?:<strong>\s*)?(?:[①-⑩]|[0-9]{1,2}\s*단계|[0-9]{1,2}\s*\.)\s*(?:<\/strong>)?\s*[\.\s]*/g,
    '<li>',
  );
  return s;
}
