...
## 2. 본문 HTML 표준 블록 구조 (v2 — 2026-04 사용자 컨펌)

### 2.0 페이지 자동 H2와 절대 중복 금지 (가장 중요)

페이지 컴포넌트(`app/(public)/welfare/[slug]/page.tsx`)가 이미 다음 H2를 자동으로 추가합니다:

- **사업 정보** (메타 테이블)
- **지원 대상** (eligibility 필드 박스)
- **신청 방법** (applicationMethod 필드 박스)
- **필요 서류** (requiredDocuments 필드 박스)
- **상세 설명** (content 필드 박스)
- **자주 묻는 질문** (faqs 배열 박스)
- **관련 정책** (relatedPolicies 자동)

따라서 **content 필드 안에 위 H2들을 다시 넣으면 중복**으로 보임. content는 페이지가 안 만드는 H2 4개만.

또한 페이지 섹션 순서는 **사업정보 → CTA1 → 지원대상 → 신청방법 → 필요서류 → CTA2 → 상세설명 → FAQ → 관련정책**.

### 2.1 content 필드 — 4섹션 구조 (페이지의 \"상세 설명\" 박스 안에 표시)
...
<div class="callout callout-info">
  <ul>
    <li>시행기관은 ...입니다.</li>
    <li>지원 대상은 ... 입니다.</li>
    <li>지원금액은 <strong>최대 OOO만 원</strong>입니다.</li>
    <li>신청기간은 ... 입니다.</li>
    <li>처리기간은 ... 입니다.</li>
  </ul>
</div>
...
<h2>지원 내용</h2>
<p>{4~6 문장 본문 + 표(<table class="policy-table">) 또는 불릿. 핵심 숫자 + 사용 방법 + 차별점.}</p>

<h2>신청 후 처리 절차</h2>
<ul>
  <li>{단계 1 소요일 — 주체와 핵심 행동}</li>
  <li>{단계 2}</li>
  ...
</ul>
...
<ul>
  <li>운영 부처는 ...입니다.</li>
  <li>YYYY년 도입 ... .</li>
  <li>공식 신청처는 <a href="..." target="_blank" rel="noopener noreferrer">...</a> 입니다.</li>
  <li>대표문의는 ☎ ... 입니다.</li>
</ul>
<p><em>마지막 업데이트 YYYY년 X월 기준.</em></p>
```

#### 절대 금지 (content 안에 넣으면 중복)
- `<h2>지원 대상</h2>` / `<h2>신청 방법</h2>` / `<h2>제출 서류</h2>` / `<h2>필요 서류</h2>`
- `<h2>자주 묻는 질문</h2>` / `<h2>함께 보면 좋은 정책</h2>` / `<h2>FAQ</h2>`
- `<strong>정책 한눈에 보기</strong>` / `<strong>한눈에 보기</strong>` (callout 안 헤더)
- em-dash `—` (\"지원 내용 — 무엇을 받을 수 있나요?\" 같은 부제목 형식)

### 2.2 eligibility 필드 — H2 없이 ul 한 덩어리

```html
<ul>
  <li>✅ 만 19~34세 (군 복무 시 최대 만 39세)</li>
  <li>✅ 신청일 기준 OO에 1년 이상 거주</li>
  <li>✅ 미취업자 또는 주 30시간 이하 근로</li>
  <li>✅ 가구 중위소득 150% 이하</li>
  <li>❌ 정규직, 사업자, 군 복무 중인 자는 제외</li>
</ul>
```

#### 절대 금지
- `<strong>연령:</strong>` / `<strong>거주요건:</strong>` 등 콜론 라벨
- 도입부 \"지원 대상은 다음과 같습니다:\" / \"~는 ~입니다:\"
- ul 뒤 추가 `<p>` 안내 (줄간격 큼)
- `<h2>` 또는 `<h3>`

### 2.3 applicationMethod 필드 — H2 없이 ol class=\"steps\"

```html
<ol class="steps">
  <li>서울청년포털에 접속해 회원가입 후 간편인증으로 로그인합니다.</li>
  <li>모집 공고를 확인하고 신청기간·자격을 점검합니다.</li>
  <li>본인·가구 정보를 입력하고 필수 서류를 업로드합니다.</li>
  <li>약 4~6주 자격 심사 후 결과를 마이페이지로 통보받습니다.</li>
  <li>선정 시 클린카드를 수령하고 매월 자기주도 활동을 인증합니다.</li>
</ol>
```

#### 절대 금지 (ol 자동 번호와 중복)
- `<li>1단계. ...</li>` / `<li><strong>1단계.</strong> ...</li>`
- `<li>① ...</li>` / `<li>1. ...</li>`
- `<strong>` 단계 강조
- 도입부 \"신청은 다음 절차로 진행됩니다:\"

### 2.4 requiredDocuments 필드 — H2 없이 p 3단락 (분류별)

```html
<p>기본 서류로 신분증 사본, 주민등록등본 최근 3개월 이내, 건강보험 자격득실확인서, 가족관계증명서, 본인 명의 통장 사본이 필요합니다.</p>
<p>본인 상황에 따라 근로·사업·기타소득 증빙(근로계약서·원천징수영수증), 군 복무 사실 증명서, 한부모·차상위 등 특수계층 증빙을 추가로 제출합니다.</p>
<p>모든 서류는 정부24, 국민건강보험공단, 홈택스, 병무청에서 온라인 무료 발급이 가능합니다.</p>
```

#### 절대 금지
- `<strong>필수:</strong>` / `<strong>해당 시:</strong>` 콜론 라벨
- `<ul>` (prose 줄간격이 두 줄로 보여 사용자 컴플레인)
- 도입부 \"필요 서류는 다음과 같습니다:\" / \"제출 서류는 ~입니다:\"
- `<h2>` 또는 `<h3>`

### 2.5 페이지가 자동 처리하는 것들 (본문에서 신경 쓸 필요 없음)

- **신청하기 버튼**: `applyUrl` 있으면 페이지가 사업정보 직후 + 필요서류 직후에 2번 자동 표시 (본문에 `<a class="cta-apply">` 굳이 안 넣어도 됨)
- **FAQ 박스**: `faqs` 배열 6~8개 → 자동 `<details>` 박스
- **JSON-LD**: GovernmentService / BreadcrumbList / FAQPage 자동 생성
- **관련 정책**: relatedPolicies 자동 4개

### 2.6 분량 기준

- **content (평문)**: 1,500~2,500자
- **eligibility**: 200자 이상 (5~7개 li)
- **applicationMethod**: 200자 이상 (4~6단계)
- **requiredDocuments**: 150자 이상 (p 3단락)
- **excerpt**: 100~200자
- **metaDesc**: 150자 이내
- **faqs**: 6~8개
